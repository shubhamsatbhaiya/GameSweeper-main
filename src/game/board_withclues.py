import random
from .cell_2 import Cell  # Assuming the `Cell` class is imported correctly.
...
class Board:
    ...
    def __str__(self):
        # Display board state as a Minesweeper-like grid
        rows = []
        for row in self.grid:
            ...
            for cell in row:
                if cell.revealed:
                    if cell.has_mine:
                        row_representation.append("*")  # Mine
                    elif cell.neighbor_mines > 0:
                        row_representation.append(str(cell.neighbor_mines))  # Neighbor mine count
                    else:
                        row_representation.append(" ")  # Empty cell
                elif cell.flagged:
                    row_representation.append("F")  # Flagged cell
                else:
                    row_representation.append("X")  # Hidden cell
            rows.append(" ".join(row_representation))
        return "\n".join(rows)
